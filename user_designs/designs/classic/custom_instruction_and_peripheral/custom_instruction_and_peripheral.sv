...
module custom_instruction_and_peripheral (
    input  wire sys_clk,
    ...
    // Bit of a hack: SYS_RESET is only connected to
    // GBUF, the output of a GBUF can be used as
    // CLK, SR or EN of a flip-flop.
    // Therefore, convert the reset to a "fabric" reset.
    LUTFF_SS LUTFF_SS (
      .O (reset_fabric),
      .CLK (sys_clk),
      .S (reset_buf),
